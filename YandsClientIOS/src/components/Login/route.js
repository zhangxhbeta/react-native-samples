    ...
    renderRightButton(navigator, index, state) {
      return (
        <TouchableOpacity
          ...
          onPress={ () => navigator.replace(registerRoute()) }
          style={ExNavigator.Styles.barRightButton}>
          <Text style={[ExNavigator.Styles.barRightButtonText, {color: '#fff'}]}>注册</Text>
        </TouchableOpacity>
      );
    },
...
class BackButton extends React.Component {
  ...
  render() {
    return (
      <TouchableOpacity
        touchRetentionOffset={ExNavigator.Styles.barButtonTouchRetentionOffset}
        onPress={() => this.props.navigator.parentNavigator.pop()}>
        <ResponsiveImage
          {...this.props}
          sources={{
            2: {
